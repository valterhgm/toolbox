...
- **Frontend**: `trackEvent()` fire-and-forget helper, called from
  ...
  Analytics failures are always swallowed, since this must never break the tool
  itself.
...
- `MongoEventRepository`: integration test against a **real MongoDB**.
  ...
  This was a genuine third-party tooling incompatibility, not our code. Fell back to
