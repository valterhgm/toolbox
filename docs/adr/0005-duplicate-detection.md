...
3. Compare hashes pairwise via **Hamming distance** (`hammingDistance`):
   the number of differing bits. Small distance = visually similar, even
...
scan a folder, since there is no such permission model on mobile. Specifically:
...
- Where `showDirectoryPicker` exists: a "Choose a Folder to Scan" button
  ...
  (`collectFilesFromDirectory`, filtered by `isImageFilename`, since a real
